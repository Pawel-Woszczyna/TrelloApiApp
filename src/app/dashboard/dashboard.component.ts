...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  getListsData(): void {
    this.endPointService.getListsData().subscribe(
      res => {
        this.getLists = res
        // this.getLists = this.getLists.map((list: any)=>{
        //   list.data = []
        //   return list
          // return {...list, 'data':[]}
        // })
  ...
  getCardData(): void {
    this.endPointService.getCardData().subscribe(
      res => {
        // res.forEach(element => {
          // this.getLists.find((el: { id: any; })=>el.id === (element as any).idList)?.data?.push(element)
        // });
        this.getCards = res
      },
      err => {
  ...
  DeleteCard(id: string): void {
    this.endPointService.deleteCard(id).subscribe(
      res => {
        this.deleteCard = res
        this.getNewData();
      },
  ...
  EditCard(id: string, name: string): void {
    this.endPointService.editCard(id, name).subscribe(
      res => {
        this.editCard = res
        this.getNewData();
      },
  ...
  AddCard(id: string, addNewCardName: string): void {
    this.endPointService.addCard(id, addNewCardName).subscribe(
      res => {
        this.addCard = res
        this.getNewData();
      },
  ...
  AddList(idBoard: string, addNewList: string): void {
    this.endPointService.addList(idBoard, addNewList).subscribe(
      res => {
        this.addList = res
        this.getNewData();
      },
  ...
  EditList(id: string, name: string, input1: HTMLInputElement): void {
    this.endPointService.editList(id, name).subscribe(
      res => {
        this.editList = res
        this.getNewData();
      },
  ...
  DeleteList(id: string): void {
    this.endPointService.deleteList(id).subscribe(
      res => {
        this.deleteList = res
        this.getNewData();
      },
  ...
  drop(event: CdkDragDrop<string[]>) {
    if (event.previousContainer === event.container) {
      ...
      // (event.previousContainer.data[event.previousIndex] as any).idList = (event.container.data[0] as any).idList
    }
    console.log(event)
  }

  slicedData(getLists : any[]) : any[] {
    return getLists?.slice(0, 1)
  }
